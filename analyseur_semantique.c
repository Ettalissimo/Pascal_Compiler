...
typedef enum {
    ...
    INTO_ERR, DOWNTO_ERR, DDOT_ERR, INT_ERR,
} CODES_ERR;

typedef enum{
    TPROG,TCONST,TVAR,UKN
} TSYM;
...
void PROGRAM(){
    ...
    TAB_IDFS[index_idfs].TIDF = TPROG;
    index_idfs++;
...
void AFFEC(){
    ...
    result = Rechercher(sym.NOM);
    if(result==TCONST){
        printf("constante %s ne peut pas changer de valeur\n",sym.NOM);
        exit(EXIT_FAILURE);
    }
    else if(result==TPROG){
        printf("nom de programme %s non autorisé",sym.NOM);
        exit(EXIT_FAILURE);
    }
    else if(result==UKN){
        printf("identificateur %s non déclaré\n",sym.NOM);
        exit(EXIT_FAILURE);
    }
...
int main() {
    ...
    Sym_Suiv();
    PROGRAM();
    printf("analyse sémantique complète, aucune erreur!!!!\n");
